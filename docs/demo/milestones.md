...
- [x] Actual Workbench frame save captured 29 source writes with default/Nolock/serializable settings; fresh export verified the edit and temporary audit was removed. Explicit ROW-configured Workbench acceptance remains pending.
- [x] Configured Workbench save verified via fresh export while an unrelated counter row remained locked; existing read settings preserved and 29 counter increments collected after holder release. Temporary hooks/launcher removed.
- [x] Reader timeout reproduced as documented serializable-ROW/MVCC incompatibility. Table-specific MVCC/shared passed live SQL matrix and actual CLI writer/reader checks; see [reader acceptance](../research/counter-reader-acceptance.md). Manual Workbench MVCC save and committed counter read also passed while an unrelated counter update remained locked; temporary hooks and launcher removed.
- [x] Internal startup composer and bounded counter reader implemented and live-tested; over-budget samples expose no token, existing SET statements/literals preserved. See [primitives](../research/revision-primitives.md). Backend/schema integration remains pending.
- [x] DBA-reviewed revision extension SQL export and generation-bound internal samples implemented; isolated OpenROAD capture, rollback and repeat-install rejection passed. Diagnostic/backend integration pending. See [extension](../research/revision-extension.md).
...
- [x] Execution-host startup artifact lifecycle implemented and Windows-tested around import/compile, including paths with spaces and an uncommitted counter holder. Database-name scoping preserves general startup settings; effective symbol lookup, transport integration and runtime-database acceptance remain pending.
