...
Run inside the project. Remote users must install helper version 9 with
...
agreement with that baseline. It imports the prepared XML and verifies a fresh full export. Compile explicitly
with `gorak compile APP COMPONENT` when needed. Verification compares the supported readable representation. Unsupported queries
...
successful XML verification. Import or verification failure may leave changed
...
operation journal; ordinary push retries reconcile recognizable writes. See
[push and recovery](push.md) for explicit side selection.
