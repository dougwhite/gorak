...
When several styles of a field type imply different omitted values, export adds
an optional Gorak-only selector:

```xml
<tablefield name="items" gorak_style="2"/>
<tablefield name="items" gorak_style="2" fgcolor="7"/>
```

`gorak_style` is a 1-based index among styles of that field type in the effective,
ordered repository → application → frame defaults. Unrelated field types do not
count. Property-only overrides preserve positions; do not reorder or deduplicate
styles without reviewing selectors. Import restores the selected style and then
applies explicit attributes, so `fgcolor="7"` does not switch styles. The selector
is consumed by Gorak and is never sent as an OpenROAD property.

Unambiguous WML remains unchanged, including multiple styles whose omitted values
are identical. Historical WML without a selector is accepted only when all styles
imply the same omitted values. Otherwise status reports invalid source and push
stops before writes: select a known style or re-export authoritative source. Old
WML alone cannot recover the lost distinction. Invalid or out-of-range selectors
are rejected. This is a bounded WML extension; no source-format flags, ordinary
W4GL changes, XML companions or general migration are required.
...
including control property values before default suppression. Byte stability alone
is insufficient when a lossy projection could hide a property change.
